...
import org.l2s.gameserver.handler.CommunityBoardHandler;
import org.l2s.gameserver.handler.IParseBoardHandler;
...
import org.l2s.gameserver.model.actor.instance.PetInstance;
...
import org.l2s.gameserver.network.serverpackets.BuyList;
import org.l2s.gameserver.network.serverpackets.ExBuySellList;
import org.l2s.gameserver.network.serverpackets.MagicSkillUse;
import org.l2s.gameserver.network.serverpackets.ShowBoard;
...
public class HomeBoard implements IParseBoardHandler
{
	...
	private static final BiPredicate<String, PlayerInstance> COMBAT_CHECK = (command, player) ->
	{
		...
		return commandCheck && (player.isCastingNow() || player.isInCombat()  || (player.getPvpFlag() > 0) || player.isInDuel() || player.isInOlympiadMode() || player.isInsideZone(ZoneId.SIEGE) || player.isInsideZone(ZoneId.PVP));
	};
